...
#if defined(NDEBUG)
# error "Chipmonk cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Chipmonk Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // This is done last to help prevent CPMK exhaustion denial-of-service attacks.
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Force block reward to zero when right shift is undefined.
       if (halvings >= 64)
        return 0;   
     if (nHeight <= 20) {
        nSubsidy = 347715 * COIN;
    } else if ((nHeight >= 20) && (nHeight <= 870)) {
        nSubsidy = 1.5 * COIN;
    } else if ((nHeight >= 870) && (nHeight <= 1740)) {
        nSubsidy = 1 * COIN;
    } else if ((nHeight >= 1740) && (nHeight <= 4000)) {
        nSubsidy = 0.5 * COIN;
    } else if ((nHeight >= 4000) && (nHeight <= 10000)) {
        nSubsidy = 0.25 * COIN;
        ...
        nSubsidy = 0.1 * COIN;
    } else if ((nHeight >= 300000) && (nHeight <= 700000)) {
        nSubsidy = 0.08 * COIN;
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        // Helps prevent CPMK exhaustion attacks.
...
void ThreadScriptCheck() {
    RenameThread("chipmonk-scriptch");
