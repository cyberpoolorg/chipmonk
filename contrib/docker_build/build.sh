...
if [ "x$1" == "x" ] || [ "x$2" == "x" ]; then
  ...
  echo "more processes = faster build, more CPMK" >&2
  exit 1
fi
...
if [ ! -f $BITCOINPATH/src/bitcoind.cpp ]; then
  echo "First parameter doesn't seem like a chipmonk path" >&2
  echo "" >&2
  echo "./build.sh bitcoin-dir number-of-processes" >&2
  echo "           for example: ./build.sh ../ 4" >&2
  echo "more processes = faster build, more CPMK" >&2
...
docker run -v $BITCOINPATH:/chipmonk bitcoind_build /run.sh $2

cd $BITCOINPATH/out

# renaming from Bitcoin name to Bitcore name.... stupid thing
rm -f $BITCOINPATH/out/chipmonk-0.15.2-x86_64-linux-gnu-debug.tar.gz
